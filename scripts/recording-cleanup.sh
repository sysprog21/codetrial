...
quote() {
  # One id per argument, single-quoted for SQL, with any embedded quote doubled.
  # Recording ids are 22 characters of base64url and contain none of this, which
  # is exactly why it costs nothing to be sure.
...
if [ -n "$expire" ]; then
  ...
  # `deleted_by` is written now and read by the sweeper, so the tombstone says a
  # person asked rather than that a deadline arrived.
  changed=$(sqlite3 "$db" "
UPDATE recordings
