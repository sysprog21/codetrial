...
if runs delivery || runs cleanup; then
  ...
  query=$(python3 - "$filename" "$CODETRIAL_RECORDING_DRIVE_ID" <<'PY'
import sys
...
PY
  ...
  drive_file_id=$(python3 - "$work/drive-files.json" "$work/delivery.json" "$recording_id" "$filename" <<'PY'
import json
import sys
...
PY
  ...
  if runs cleanup; then
    ...
    case "$timeout" in '' | *[!0-9]*) echo "CODETRIAL_RECORDING_CLEANUP_TIMEOUT_SECONDS takes seconds" >&2; exit 2 ;; esac
    deadline=$(( $(date +%s) + timeout ))

    # The object name carries a slash the JSON API wants encoded, and it does
    # not change between polls, so encode it once rather than once a poll.
    escaped_object=$(python3 - "$object" <<'PY'
import sys
...
PY
    ...
    while :; do
      ...
      [ "$(date +%s)" -lt "$deadline" ] || { echo "cleanup timed out: Drive=$drive_status GCS=$gcs_status" >&2; exit 1; }
      sleep 5
    done
