...
| Bundle | Introduced |
|---|---|
| 15 | Both resumed and cold Gemini connections reconcile the local transcript, editor, browser test report, and the active round's framework progress. A resumed session receives the update as context; it answers at once only when the replaced connection owed a reply to a finished candidate turn, an unanswered prompt, or an outstanding tool continuation, and otherwise waits for the candidate. Cold sessions paused during recovery receive their briefing on unpause. Missing evidence records do not force completed steps to repeat. The coding silence nudge, editor review and five-minute warning state the recorded test progress: after a real run they no longer ask for tests unless the code changed since, and once the coding completion gate has passed they do not revisit tests, complexity or edge cases. A runner setup error is not a run, so the silence nudge allows retrying it and the warning still asks for tests. An editor review that goes unanswered is not owed a reply after a reconnection. A prompt that produces no output for 20 seconds returns the floor, so silence nudges and a deferred transport restart are no longer held behind it. |
