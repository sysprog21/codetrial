...
Bundle 15: live prompt 7, report prompt 11, rubric 1, report schema 2.

| Bundle | Introduced |
|---|---|
| 15 | Test is recorded from a received test run with executed cases, with source `test_event`, and only while the editor still holds the code that run executed: fewer than five non-whitespace characters of code added and fewer than five removed since, with comments left out of the count, so a fix in place or a note of the complexity keeps the run while writing the solution after running a stub, or deleting a guard the run exercised, does not. The browser sends the submitted code with the results, so a run of the starter or of another language's buffer vouches for nothing written after it. Verbal tracing, setup errors, and empty runs cannot complete Test or unlock normal coding completion; failing test cases still count as testing. A test reaction asks the interviewer to record Test only when that record would be accepted, asks for a run of the code now on screen when the candidate edited past the run, and a run that makes Test recordable reacts even inside the test-reaction cooldown. A run that could not start because the platform failed (the judge did not load or is missing, Compiler Explorer or the Python runtime was unreachable) lets the candidate's hand trace of the written code stand for Test, recorded with source `candidate_speech`, in that run's language only, until a later run says otherwise; a run that executed cases never carries that exception, and a current run of the code on screen still takes precedence. The flag is the browser's claim, as the pass counts are, so a Test recorded from `candidate_speech` marks this path in the report. The five-minute warning asks for a run rather than a description of tests, and a refused `end_interview` points the interviewer at the Run button, or at the hand trace while the runner is unavailable. |
