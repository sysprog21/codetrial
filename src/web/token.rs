...
mod tests {
    ...
    fn an_observation_grant_expires_whether_or_not_the_sweep_has_run() {
        ...
        // The last second a grant is good and the first it is not, and the
        // sweep read at the same edge: expiring on the second it runs is still
        // expiring, and a grant still inside its life is not swept out from
        // under whoever is watching with it.
        for (granted, trigger, left) in
            [(0, TOKEN_TTL_SECONDS, 1), (100, TOKEN_TTL_SECONDS + 50, 2)]
        {
            let mut rooms = RoomAuthorizations::default();
