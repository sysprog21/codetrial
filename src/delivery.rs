...
mod tests {
    ...
    fn dates_are_rfc_3339_in_utc() {
        ...
        // against dates a leap year and a century get wrong when it is written
        // by hand.
    ...
    fn object_names_survive_their_slashes() {
        // The JSON API takes the whole object name as one path segment, so a
        // prefix that arrived unescaped would name a different object, or none.
        assert_eq!(
    ...
    fn a_service_account_without_a_key_is_refused_at_startup() {
        ...
        // Base64 that decodes and is not a key. Without the `ring` check this
        // was accepted at startup and failed at the first delivery, which is
        // the failure the doc comment above `new` promises not to have.
